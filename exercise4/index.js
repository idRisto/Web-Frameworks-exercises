...
let products = [{
    id: uuidv4(),
    name: 'Testi',
    manufacturer: 'TestiOY',
    category: 'computers',
    description: 'TestiTietokone',
    price: 99.99
}]
...
app.post('/invoices/:userId', (req,res) => {
    ...
    for (let i = 0; req.body.id.length; i++) {
        total = total + products[products.findIndex(d => d.id == req.params.userId)]
    }
    for (let i = 0; req.body.id.length; i++) {
        allProducts[i] = products[products.findIndex(d => d.id == req.params.userId)]
    }
    invoices.push({
        id: uuidv4(),
